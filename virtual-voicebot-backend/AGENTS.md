...
- Codex は Approved 以降の段取り更新（§3.5）のみ可能
- ステアリングの仕様変更（§5/§6/§2）は Claude Code が担当
...
> **注意:** ステアリングの新規作成・Review 時の修正は Claude Code が担当。
> Codex は `Status: Approved` のステアリングに従って実装するのみ。
