...
### 1.3.1 ステアリングファイルの修正権限

| ステータス | Claude Code | Codex | 備考 |
|-----------|-------------|-------|------|
| **新規作成** | ✓ 担当 | **❌ 禁止** | Draft 作成は Claude Code の専任 |
| Draft（修正） | ✓ 担当 | ❌ 禁止 | 初期仕様策定フェーズ |
| Review | ❌ 禁止 | ✓ 修正可 | レビュー指摘への対応（レビュー必須） |
| Approved | - | ✓ 段取り更新 | 実装進捗の記録 |
| Merged | - | - | アーカイブ（修正不要） |

**重要:**
- **Codex による新規ステアリングファイルの作成は禁止**
- Codex は既存ステアリングの修正のみ可能
- 修正時は Claude Code またはオーナーのレビューを受けること
- Status の勝手な変更は禁止（Status 更新は人間が判断）

詳細は [AGENTS.md（root）§ドキュメント更新の扱い](../../AGENTS.md#ドキュメント更新の扱い必須) を参照。
...
| Review | オーナー | Draft 完了・レビュー開始時 |
| Approved | オーナー/PL/PO | レビュー承認時 |
| Merged | 担当者 | PR マージ後 |

**重要:**
- AI（Claude Code/Codex）は Status を勝手に変更しない
- Status 変更は人間の判断で行う
...
#### 8.1.1 Review時の対応（レビュー指摘への修正）

ステアリングが `Status: Review` の場合、レビュー指摘に基づく修正を行う：

1. **レビュー指摘の確認**: ステアリング §3.3（レビュー）の指摘内容を読む
2. **差分仕様の修正**: §5 を最小差分で修正（レビュー指摘に対応）
3. **受入条件の修正**: §6 を修正（必要に応じて）
4. **未確定点の解消**: §7 の質問に回答（必要に応じて）
5. **レビュー記録の更新**: §3.3 に修正内容・対応日を記録
6. **PR 作成**: 修正内容を PR にして、Claude Code またはオーナーに再レビュー依頼

**禁止事項:**
- **ストーリー（§2）の変更**（Why の変更は Issue で合意すべき）
- Draft ステータスのステアリングの修正
- Codex による新規ステアリングファイルの作成
- Status の勝手な変更
- 全文再生成（必ず最小差分で修正）

**レビュー要件:**
- 修正内容は必ず Claude Code またはオーナーのレビューを受けること
- レビュー承認後、オーナーが Status を Approved に更新

#### 8.1.2 実装時の対応（Approved以降）

ステアリングが `Status: Approved` の場合、実装を行う：
...
### 8.3 コードレビュー（CodeRabbit）

コードレビューは **CodeRabbit** が自動で実施する。
...
**Codex の責務**:
- CodeRabbit の指摘を確認し、必要に応じて修正
- 指摘が仕様に関わる場合は Claude Code に相談
