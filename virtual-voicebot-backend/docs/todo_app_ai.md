...
## 挙動を変えないリファクタ
- [ ] app モジュールの骨（README と stub）を用意し、対話状態管理の責務をコメントで明示する（呼び出しはまだ行わない）。
- [ ] ai モジュールを asr/llm/tts のサブモジュール境界だけ切り出し、既存関数を薄いラッパで包む（挙動は同じ）。
- [ ] session の `handle_bot_pipeline` を関数分割し、AI 呼び出し部分に「app/ai へ移行予定」コメントを付与する（挙動は維持）。
- [ ] voice_bot_flow に沿ったイベント名のドラフトを docs/app/ai に追記し、現行コードとの差分を明示する（コードはまだ触らない）。
- [ ] 既存の ai 関数 I/F（ファイルパス渡し等）の前後で責務境界をコメントに反映し、将来のチャネルI/Fに備える。
