...
- [x] session→app のイベントチャネルを追加し、音声バッファ完了/通話開始/終了通知を app に送るようにする。
- [x] app→ai::{asr,llm,tts} の呼び出し経路を実装し、AI呼び出しを session から完全排除する。
- [x] ai::{asr,llm,tts} を独立I/F（PCM入力/テキスト入出力）に分割し、ストリーミング/チャンク単位の処理を導入する。（MVPでは一括WAVのまま、NEXTでストリーミング実装）
