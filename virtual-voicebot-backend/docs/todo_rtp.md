...
- [ ] 送信経路を session→rtp→transport に切り替え、session内の build_rtp_packet/UdpSocket 使用を廃止して rtp がソケットと Seq/Timestamp/SSRC を管理する。
- [ ] ストリーム管理を導入し、(セッションID/SSRC/リモートアドレス)をキーに Seq/Timestamp/SSRC を保持する（MVPは単一SSRC/PCMUのみ）。
- [ ] payload type別処理の抽象を追加し、PCMU以外は未実装として分岐（将来拡張に備える）。
- [ ] 簡易ジッタ/遅延廃棄ポリシーを rtp 内に実装し、ASRへのPCM通知を統一する。
- [ ] RTCP入口を接続し、受信時にイベント/ログを出す。送信はI/Fを呼べる形にしてタイマ開始だけ実装（SR/RR生成は後続）。
- [ ] 新経路で INVITE→ACK→RTP往復のスモークを通し、旧直結処理を削除する。
