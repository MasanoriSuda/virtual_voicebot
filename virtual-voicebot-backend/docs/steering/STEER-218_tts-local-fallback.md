...
- [ ] URL 形式（base URL）の採用に合意しているか
- [ ] `TTS_LOCAL_SERVER_BASE_URL` の互換維持（デフォルト `http://localhost:50021`）に合意しているか
- [ ] タイムアウトが「段全体（2 リクエスト合計）」方式に合意しているか
- [ ] タイムアウト値（5000/10000 ms）が妥当か
- [ ] `TTS_RASPI_ENABLED=false` デフォルトに合意しているか
