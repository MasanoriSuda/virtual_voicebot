...
<!--
  ...
  - イシュー番号: GitHub Issue の番号（例: 080）
  - slug: 英小文字、ハイフン区切り、20文字以内

  例:
    - STEER-080_cancel-handling.md
    - STEER-081_dtmf-detection.md
    - STEER-082_http-range.md
  ...
  例外（Claude Code 可）:
  - コード参照の行番号誤り・ファイル名誤り・実装挙動の不正確な表現などの事実誤記修正
    ただし「ストーリーの意図を変えない」かつ「オーナーの AskUserQuestion 許可」が必要
...
| docs/test/system/ST-xxx.md | 追加 / 修正 | |

### 4.2 影響するコード

| モジュール | 変更種別 | 概要 |
|-----------|---------|------|
| src/xxx/mod.rs | 追加 / 修正 | |
