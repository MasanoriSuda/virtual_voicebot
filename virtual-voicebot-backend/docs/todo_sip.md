...
### 挙動を変えないリファクタ
- [x] `process_sip_datagram` まわりの責務境界をコメントで明示し、現行と同じイベント出力を保ったままパース→イベント化の流れを整理する。
- [x] コアヘッダ抽出（Via/From/To/Call-ID/CSeq など）をトランザクションで再利用できるようヘルパを分割し、既存挙動は維持する。
- [x] `SipEvent` と session 側で受けるイベントのフィールド/命名を設計ドキュメントに合わせて整合させ、必要ならプレースホルダ型を導入する（動作は変えない）。
