...
use crate::protocol::session::types::{SessionRefresher, SessionTimerInfo};
        ...
        tokio::spawn(async move {
            tokio::select! {
                _ = tokio::time::sleep(duration) => {
                    let _ = tx.send(crate::protocol::session::types::SessionControlIn::RingDurationElapsed).await;
                }
                _ = &mut cancel_rx => {}
            }
        });
