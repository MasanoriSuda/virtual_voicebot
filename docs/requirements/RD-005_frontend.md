...
| 依存先 | 種別 | 説明 |
|--------|------|------|
| [contract.md](../contract.md) | 必須 | Backend ↔ Frontend API 契約 |
...
- [x] Q1: 文字起こしデータの形式は？
  - **回答**: 3点セットで保存
    1. 生データ（ベンダー出力）— 丸ごと保存
    2. UI用正規化 JSON — 共通フォーマット
    3. 表示用テキスト（SRT/VTT）— プレイヤー連携用

- [x] Q2: Dashboard の KPI は Backend から取得か、Frontend で集計か？
  - **回答**: Backend 集計 → Frontend は取得・表示のみ

- [x] Q3: 通話データの永続化は Frontend 側 DB か、Backend API 経由か？
  - **回答**: Frontend 側で Server Sync 機構を構築し、ローカル DB に永続化

- [x] Q4: v0 プロトタイプの本リポジトリへの合流方法は？
  - **回答**: たたき台として利用中、合流は別途対応
